...
from sqlalchemy import Integer, String, ForeignKey, TIMESTAMP, Index, Enum, Text
...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(Enum("user", "admin", name="user_role"), server_default="user")
    created_at: Mapped[str] = mapped_column(TIMESTAMP)
    updated_at: Mapped[str] = mapped_column(TIMESTAMP)
    task = relationship("Task", back_populates="user", lazy="selectin")
...
class Task(Base):
    ...
    status: Mapped[str] = mapped_column(Enum("pending", "in_progress", "completed", "failed", name="task_status", default="pending"))
    ...
    user = relationship("User", back_populates="task", lazy="selectin")
    epubs = relationship("Epub", back_populates="task", lazy="selectin")
    images = relationship("Images", back_populates="task", lazy="selectin")

class Epub(Base):
    __tablename__ = "Epubs"
    __table_args__ = (
        Index("idx_epubs_task_id", "task_id"),
    )
    ...
    status: Mapped[str] = mapped_column(Enum("uploaded", "processing", "completed", "failed", name="epub_status", default="uploaded"))
    created_at: Mapped[str] = mapped_column(TIMESTAMP)
    updated_at: Mapped[str] = mapped_column(TIMESTAMP)
    task = relationship("Task", back_populates="epubs", lazy="selectin")
    images = relationship("Images", back_populates="epub", lazy="selectin")
...
class Images(Base):
    ...
    created_at: Mapped[str] = mapped_column(TIMESTAMP)
    updated_at: Mapped[str] = mapped_column(TIMESTAMP)
    epub = relationship("Epub", back_populates="images", lazy="selectin")
    task = relationship("Task", back_populates="images", lazy="selectin")
    description = relationship("ImageDescription", back_populates="image", lazy="selectin")
...
class ImageDescription(Base):
    ...
    model_ia_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("ModelsIA.id"), nullable=True)
    ...
    created_at: Mapped[str] = mapped_column(TIMESTAMP)
    updated_at: Mapped[str] = mapped_column(TIMESTAMP)
    image = relationship("Images", back_populates="description", lazy="selectin")
    modelIA = relationship("ModelsIA", back_populates="descriptions", lazy="selectin")
...
class ModelsIA(Base):
    ...
    created_at: Mapped[str] = mapped_column(TIMESTAMP)
    updated_at: Mapped[str] = mapped_column(TIMESTAMP)
    descriptions = relationship("ImageDescription", back_populates="modelIA", lazy="selectin")
