...
from ..database import Task, Images, ImageDescription
...
async def get_validated_description(session: AsyncSession, task_id_redis: str) -> dict[str, str]:
    result = await session.execute(
        ...
        .where(Task.task_id_redis == task_id_redis,
               (ImageDescription.validated_by_human == True) | (ImageDescription.is_written_by_human == True))
    )
    ...
    descriptions_map = {}
    for task, img, desc in rows:
        descriptions_map[img.image_file_name] = desc.description_text
            
    return descriptions_map

async def add_descriptions(session: AsyncSession, task_id_redis: str, epub_path: str, output_path:str) -> str:
